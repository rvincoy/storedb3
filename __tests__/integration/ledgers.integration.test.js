...
// in controllers/ledgers.js against an actual persisted product, not a mock.
describe('ledgers controller (integration)', () => {
  ...
  test('createLedger decrements the real product stock and records SoldBy', async () => {
    ...
    const req = { user: { id: userId }, body: baseLedgerBody() };
    const res = mockRes();
  ...
  test('createLedger rejects a sale that exceeds real available stock', async () => {
    const req = { user: { id: new ObjectId().toString() }, body: baseLedgerBody({ Quantity: STARTING_STOCK + 1 }) };
    const res = mockRes();
  ...
  test('updateLedger adjusts real stock by the quantity delta', async () => {
    const userId = new ObjectId().toString();
    const createReq = { user: { id: userId }, body: baseLedgerBody({ Quantity: 4 }) };
