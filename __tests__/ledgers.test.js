...
describe('ledgers controller', () => {
  ...
  function baseLedgerBody(overrides = {}) {
    return {
      ProductName: 'Widget',
      Description: 'd',
      Category: 'c',
  ...
  test('createLedger decrements product stock and records SoldBy', async () => {
    ...
    const req = { user: { id: userId }, body: baseLedgerBody({ ProductID: productId.toString() }) };
    const res = mockRes();
  ...
  test('createLedger rejects a sale that exceeds available stock', async () => {
    ...
    const req = { user: { id: userId }, body: baseLedgerBody({ ProductID: productId.toString(), Quantity: 999 }) };
    const res = mockRes();
  ...
  test('createLedger 404s when the referenced product does not exist', async () => {
    const req = { user: { id: userId }, body: baseLedgerBody({ ProductID: new ObjectId().toString() }) };
    const res = mockRes();
  ...
  test('createLedger rejects missing required fields', async () => {
    const req = { user: { id: userId }, body: { ProductName: 'Widget' } };
    const res = mockRes();
  ...
  test('deleteLedger removes an existing ledger entry', async () => {
    ...
    await ledgersController.deleteLedger({ params: { id: String(ledgerId) } }, res);
